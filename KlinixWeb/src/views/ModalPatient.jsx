...
export default function ModalPatient({ onClose, modo, paciente = {}, refrescarPacientes }) {
    ...
    useEffect(() => {
        ...
        if (paciente?.Department) {
            const depByName = departamentos.find((d) => String(d?.nombre).toLowerCase() === String(paciente.Department).toLowerCase());
            if (depByName?.id) {
        ...
        if (paciente?.City_Id) {
            const depByCity = departamentos.find((d) => Array.isArray(d?.ciudades) && d.ciudades.some((c) => String(c.id) === String(paciente.City_Id)));
            if (depByCity?.id) {
                setDepartamentoSeleccionado(String(depByCity.id));
            }
        }
    }, [departamentos, paciente, departamentoSeleccionado]);
