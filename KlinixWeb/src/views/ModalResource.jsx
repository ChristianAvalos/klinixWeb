...
export default function ModalResource({ onClose, modo, resource = {}, refrescarResources }) {
    const [nombre, setNombre] = useState(resource.FirstName || '');
    const [apellido, setApellido] = useState(resource.LastName || '');
    const [direccion, setDireccion] = useState(resource.Address || '');
    const [celular, setCelular] = useState(resource.CellPhoneNumber || '');
    const [soportaWhatsapp, setSoportaWhatsapp] = useState(resource.SupportWhatsapp === "1");
    const [telefono, setTelefono] = useState(resource.PhoneNumber || '');
    const [correo, setCorreo] = useState(resource.Email || '');
    const [ciudadSeleccionado, setCiudadSeleccionado] = useState(resource.City_Id || '');
